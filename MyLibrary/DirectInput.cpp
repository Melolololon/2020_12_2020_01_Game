...
void DirectInput::setMatrix(DirectX::XMMATRIX viewMatrix, DirectX::XMMATRIX projectionMatrix,float nearNumber, float farNumber)
{
	viewMat = viewMatrix;
...
void DirectInput::getMouse3DLine(Vector3 cameraPos, Vector3& nearPoint, Vector3& farPoint)
{
	...
	//逆行列
	DirectX::XMMATRIX revViewPortMatrix;
	DirectX::XMMATRIX revViewMatrix;
	DirectX::XMMATRIX revProjectionMatrix;
	...
	revViewPortMatrix = DirectX::XMMatrixInverse(nullptr, viewportMatrix);
	revViewMatrix = DirectX::XMMatrixInverse(nullptr, viewMat);
	revProjectionMatrix = DirectX::XMMatrixInverse(nullptr, projectionMat);

#pragma region Z = 0

	//z = 0の場合
	mousePosMatrix = DirectX::XMMatrixTranslation(mousePos.x, mousePos.y, 0);
	//スクリーン座標をワールド座標に
	worldMousePosMatrix = DirectX::XMMatrixIdentity();

	worldMousePosMatrix *= mousePosMatrix;
	worldMousePosMatrix *= revViewPortMatrix;
	worldMousePosMatrix *= revProjectionMatrix;
	...
	worldMousePosMatrix *= revViewMatrix;
...
#pragma region z = 1
	...
	//z = 1の場合
	mousePosMatrix = DirectX::XMMatrixTranslation(mousePos.x, mousePos.y, 1);
